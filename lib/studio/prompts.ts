...
export function buildInitialStudioPrompt({ projectName, topicId, userPrompt }: InitialPromptInput) {
...
export function buildFollowupStudioPrompt(topicId: string, userPrompt: string) {
  const workspace = studioWorkspace(topicId)
  return [
    '[WTT_STUDIO_CONTINUE]',
    `workspace=${workspace}`,
...
export function buildPublishPrompt(topicId: string) {
  const workspace = studioWorkspace(topicId)
  return [
    '[WTT_STUDIO_PUBLISH]',
    `workspace=${workspace}`,
...
export function buildGithubPrompt(topicId: string, projectName: string) {
  const workspace = studioWorkspace(topicId)
  return [
    '[WTT_STUDIO_GITHUB]',
    `project_name=${projectName}`,
...
export function buildPreviewPrompt(topicId: string) {
  const workspace = studioWorkspace(topicId)
  return [
    '[WTT_STUDIO_PREVIEW]',
    `workspace=${workspace}`,
