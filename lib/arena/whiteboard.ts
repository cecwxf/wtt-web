...
export function makeWhiteboardPrompt(challenge: Challenge, locale: WhiteboardLocale, stepMode = false) {
  ...
  const limits = zh
    ? '每一步的 markdown 必须包含 2-4 句解释文字、必要公式/指标定义、一个紧凑表格或要点列表；Mermaid 只画结构，不承载长文字。涉及算法复杂度、概率/损失函数、向量检索、系统容量、延迟或评测指标时，公式不能省。原理性架构图必须清晰：按数据流从左到右或从上到下画，区分输入 token/状态、核心模块、缓存或内存、输出/评测。涉及 Transformer decode、Attention、KV cache、MoE、RAG、推理调度等过程时，要把过程拆成可动画播放的阶段：输入嵌入→QKV/注意力→KV cache 读写→FFN/残差归一化→logits/采样→next token 回填，并在 markdown 中解释关键公式，例如 attention(Q,K,V)=softmax(QK^T/sqrt(d_k))V。Mermaid 每步 3-7 个节点、2-6 条边，节点名必须来自本轮回答的具体概念，并用 classDef/class 至少区分 3 类颜色：输入/数据、核心/模型、缓存/状态、风险/决策、指标/运维。'
    : 'Each step markdown must include 2-4 explanatory sentences, required formulas/metric definitions, and one compact table or bullet list. Mermaid should show structure only, not long prose. Do not omit formulas for algorithm complexity, probability/loss functions, vector retrieval, capacity, latency, or evaluation metrics. Principle architecture diagrams must be clear: draw data flow left-to-right or top-to-bottom, distinguishing input tokens/state, core modules, cache or memory, and output/evaluation. For Transformer decode, Attention, KV cache, MoE, RAG, or inference scheduling, break the process into animation-friendly phases: input embedding -> QKV/attention -> KV cache read/write -> FFN/residual norm -> logits/sampling -> next-token feedback, and explain key formulas in markdown, for example attention(Q,K,V)=softmax(QK^T/sqrt(d_k))V. Keep each Mermaid diagram to 3-7 nodes and 2-6 edges, with node names from this answer, and use classDef/class for at least three semantic colors: input/data, core/model, cache/state, risk/decision, metrics/ops.'
  const example = JSON.stringify({
    ...
    steps: [
      ...
      },
      {
        ...
        title: '2. 架构 / 概念分析',
        markdown: '原理题要把结构和公式对齐：先画数据流，再解释每条边代表的张量或状态变化。以 Transformer decode 为例，注意力核心公式是：\n\n$$Attention(Q,K,V)=softmax(\\frac{QK^T}{\\sqrt{d_k}})V$$\n\n| Component | State change |\n| --- | --- |\n| KV cache | Append current K/V, reuse history |\n| Sampler | Convert logits to next token |',
        mermaid: 'flowchart LR\n  Token["input token"] --> Embed["embedding + position"]\n  Embed --> QKV["Q/K/V projection"]\n  QKV --> Cache["KV cache read/write"]\n  Cache --> Attn["causal attention"]\n  Attn --> Logits["logits + sampler"]\n  Logits --> Next["next token feedback"]\n  classDef input fill:#dbeafe,stroke:#2563eb,color:#0f172a;\n  classDef core fill:#ede9fe,stroke:#7c3aed,color:#2e1065;\n  classDef cache fill:#fef3c7,stroke:#d97706,color:#451a03;\n  classDef metric fill:#dcfce7,stroke:#16a34a,color:#052e16;\n  class Token,Embed input;\n  class QKV,Attn core;\n  class Cache cache;\n  class Logits,Next metric;',
      },
      {
        stage: 'decomposition',
        title: '3. 题目要点拆解',
