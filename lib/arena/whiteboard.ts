...
export function makeWhiteboardPrompt(challenge: Challenge, locale: WhiteboardLocale, stepMode = false) {
  ...
  const limits = zh
    ? '顶层 mermaid 必须是一张最终总图，放在 JSON 的 mermaid 字段，不要只把图放进 steps；这张图要能在动画结束后独立表达完整结构。每一步的 markdown 必须包含 2-4 句解释文字、必要公式/指标定义、一个紧凑表格或要点列表；步骤 mermaid 只有在需要局部细节时才给，不能重复总图。所有原理性解释都必须这样处理，不只限于 LLM：数学公式、物理过程、算法机制、操作系统/网络/编译器/数据库、硬件/芯片/电路、AI 模型、工程架构都要给“公式或不变量 + 最终总图 + 每步解释”。原理性架构图必须清晰：按数据流、状态流、控制流、能量/力/信号流或推导依赖从左到右或从上到下画，区分输入/已知量、核心变换、状态/缓存/中间量、输出/结论、误差/边界条件。凡是有公式推导，要把符号定义、关键变形、适用条件、边界情况写在 markdown；凡是有机制过程，要把过程拆成可解释阶段，例如：定义变量→建立关系→代入变换→得到中间量→检查边界→输出结论。顶层 Mermaid 控制在 5-9 个节点、4-8 条边，节点名必须来自本轮回答的具体概念，并用 classDef/class 至少区分 3 类颜色：输入/数据、核心/变换、状态/中间量、风险/边界、指标/结论。'
    : 'The top-level mermaid field must contain one final overview diagram; do not put diagrams only inside steps. This final diagram must remain meaningful after animation ends. Each step markdown must include 2-4 explanatory sentences, required formulas/metric definitions, and one compact table or bullet list. Step-level Mermaid is optional and should only add local detail, never duplicate the overview. Apply this to every principle explanation, not only LLMs: math formulas, physics processes, algorithms, OS/network/compiler/database mechanisms, hardware/chip/circuit design, AI models, and engineering architecture must include "formula or invariant + final overview diagram + step explanation". Principle architecture diagrams must be clear: draw data flow, state flow, control flow, energy/force/signal flow, or derivation dependencies left-to-right or top-to-bottom, distinguishing input/known quantities, core transformation, state/cache/intermediate value, output/conclusion, and error/boundary condition. For formula derivations, define symbols, key transformations, assumptions, and boundary cases in markdown. For mechanisms, break the process into explainable phases, such as: define variables -> establish relation -> substitute/transform -> compute intermediate -> check boundary -> output conclusion. Keep the top-level Mermaid diagram to 5-9 nodes and 4-8 edges, with node names from this answer, and use classDef/class for at least three semantic colors: input/data, core/transform, state/intermediate, risk/boundary, metric/conclusion.'
  const example = JSON.stringify({
    ...
    steps: [
      ...
      },
      {
        ...
        mermaid: 'flowchart LR\n  Known["known quantities"] --> Model["core relation / mechanism"]\n  Model --> Transform["derive or transform"]\n  Transform --> State["intermediate state"]\n  State --> Boundary{"boundary check"}\n  Boundary --> Result["conclusion / metric"]\n  classDef input fill:#dbeafe,stroke:#2563eb,color:#0f172a;\n  classDef core fill:#ede9fe,stroke:#7c3aed,color:#2e1065;\n  classDef state fill:#fef3c7,stroke:#d97706,color:#451a03;\n  classDef metric fill:#dcfce7,stroke:#16a34a,color:#052e16;\n  class Known input;\n  class Model,Transform core;\n  class State,Boundary state;\n  class Result metric;',
      },
      {
        stage: 'decomposition',
        title: '3. 题目要点拆解',
