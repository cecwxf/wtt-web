...
export function makeWhiteboardPrompt(challenge: Challenge, locale: WhiteboardLocale, stepMode = false) {
  ...
  const limits = zh
    ? '每一步的 markdown 必须包含 2-4 句解释文字、必要公式/指标定义、一个紧凑表格或要点列表；Mermaid 只画结构，不承载长文字。所有原理性解释都必须这样处理，不只限于 LLM：数学公式、物理过程、算法机制、操作系统/网络/编译器/数据库、硬件/芯片/电路、AI 模型、工程架构都要给“公式或不变量 + 过程图 + 每步解释”。原理性架构图必须清晰：按数据流、状态流、控制流、能量/力/信号流或推导依赖从左到右或从上到下画，区分输入/已知量、核心变换、状态/缓存/中间量、输出/结论、误差/边界条件。凡是有公式推导，要把符号定义、关键变形、适用条件、边界情况写在 markdown；凡是有机制过程，要把过程拆成可动画播放的阶段，例如：定义变量→建立关系→代入变换→得到中间量→检查边界→输出结论。Mermaid 每步 3-7 个节点、2-6 条边，节点名必须来自本轮回答的具体概念，并用 classDef/class 至少区分 3 类颜色：输入/数据、核心/变换、状态/中间量、风险/边界、指标/结论。'
    : 'Each step markdown must include 2-4 explanatory sentences, required formulas/metric definitions, and one compact table or bullet list. Mermaid should show structure only, not long prose. Apply this to every principle explanation, not only LLMs: math formulas, physics processes, algorithms, OS/network/compiler/database mechanisms, hardware/chip/circuit design, AI models, and engineering architecture must include "formula or invariant + process diagram + step explanation". Principle architecture diagrams must be clear: draw data flow, state flow, control flow, energy/force/signal flow, or derivation dependencies left-to-right or top-to-bottom, distinguishing input/known quantities, core transformation, state/cache/intermediate value, output/conclusion, and error/boundary condition. For formula derivations, define symbols, key transformations, assumptions, and boundary cases in markdown. For mechanisms, break the process into animation-friendly phases, such as: define variables -> establish relation -> substitute/transform -> compute intermediate -> check boundary -> output conclusion. Keep each Mermaid diagram to 3-7 nodes and 2-6 edges, with node names from this answer, and use classDef/class for at least three semantic colors: input/data, core/transform, state/intermediate, risk/boundary, metric/conclusion.'
