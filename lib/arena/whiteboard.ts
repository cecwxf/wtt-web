...
export function makeWhiteboardPrompt(challenge: Challenge, locale: WhiteboardLocale, stepMode = false) {
  ...
  const visualHtmlRequirements = zh
    ? 'HTML 视觉质量硬性要求：默认使用浅色高质感研究白板，不要黑色大块动画，不要默认黑色 SVG fill，不要浏览器默认字体。主 SVG 必须是大图，不是缩略图；viewBox 建议至少 1200x560，svg 宽度 100%，图内节点字号 18-28px，公式字号 20-30px，主图高度不少于 420px。必须定义 :root 色彩 token 和字体 token；至少 8 个具名颜色变量，例如纸张、墨色、蓝、琥珀、绿、珊瑚、淡紫、网格线。字体优先使用本地高级栈："Avenir Next","Helvetica Neue",sans-serif；公式/标题可用 Georgia,"Times New Roman",serif；数据用 SFMono-Regular,Cascadia Code,monospace；禁止 Inter/Roboto/Arial/system-only。所有 SVG 节点、箭头、路径都必须显式 fill/stroke，不能产生黑色 blob；路径动画结束后箭头仍完整可见。必须包含清晰标题、核心图解、公式说明、简单示例、动画说明、结论/检查清单六类内容，但不要使用 Markdown 的四段标题。图必须配文字说明：每个关键节点或公式旁边要有 1-2 句解释，说明“它代表什么、为什么这样变、边界条件是什么”。公式必须逐项解释符号、单位/含义、适用条件，并给一个最小数字或业务例子代入。动画必须真实存在：使用 @keyframes、animation-delay、stroke-dasharray/stroke-dashoffset 或 transform/opacity，让路径、节点或公式按步骤出现；同时提供“如何读这个动画”的文字说明。整体不能只有图，文字解释不少于 6 个说明块。'
    : 'HTML visual quality requirements: default to a light premium research-board aesthetic; do not create black-block animations, default black SVG fills, or browser-default typography. The main SVG must be large, not a thumbnail; use a viewBox of at least about 1200x560, svg width 100%, node font size 18-28px, formula font size 20-30px, and main diagram height at least 420px. Define :root color and font tokens; include at least 8 named color variables such as paper, ink, blue, amber, green, coral, lavender, grid. Use distinctive local font stacks: "Avenir Next","Helvetica Neue",sans-serif; Georgia,"Times New Roman",serif for formula/title accents; SFMono-Regular,Cascadia Code,monospace for data. Do not use Inter/Roboto/Arial/system-only. Every SVG node, arrow, and path must explicitly set fill/stroke; no black blobs; animated arrows/paths must remain fully visible after animation. Include six content types: clear title, core diagram, formula explanation, simple example, animation explanation, and conclusion/checklist, but do not use the four Markdown step headings. Every key node or formula needs 1-2 adjacent explanatory sentences explaining what it means, why the transformation is valid, and its boundary conditions. Explain every formula symbol, unit/meaning, applicability, and include one minimal numeric or business example. Animation must be real: use @keyframes, animation-delay, stroke-dasharray/stroke-dashoffset, or transform/opacity so paths/nodes/formulas appear step by step; also include text explaining how to read the animation. The page cannot be diagram-only; include at least 6 explanatory text blocks.'
  const limits = zh
    ? `必须同时输出两种白板产物：A) markdown/mermaid 图文白板，B) html 动画白板。Markdown 负责四步讲解；HTML 是另一套面向视觉理解的产物，必须先阅读你本轮学习 Coach 自然语言回答，抽取其中真正需要可视化的公式、原理、方法、流程、架构、状态变化、变量关系、复杂度/指标和 trade-off，然后重新设计为一张详细的 SVG/HTML 动画讲解页。HTML 禁止使用“Socratic/架构和概念/问题分解/完整答案”四段标题，禁止照搬 markdown steps，禁止通用模板、占位文字或“Known/Relation/Transform”套话。HTML 必须至少包含：1) 一个本题专属 SVG 主图，表达公式推导、变量流、状态流、控制流、数据流或组件依赖；2) 一个符号/组件/指标定义表；3) 一个过程动画，用 CSS keyframes 高亮从输入到中间状态再到结论的路径；4) 一个最后静止可读的总结图或矩阵。${visualHtmlRequirements} 顶层必须有 mermaid 总图，顶层必须有 html 字段；steps[0] 必须有局部诊断/流程 mermaid，architecture_concepts 步要给局部架构图，decomposition 或 complete_answer 步要给局部流程图。每一步的 markdown 必须包含 2-4 句解释文字、必要公式/指标定义、一个紧凑表格或要点列表。所有原理性解释都必须这样处理，不只限于 LLM：数学公式、物理过程、算法机制、操作系统/网络/编译器/数据库、硬件/芯片/电路、AI 模型、工程架构都要给“公式或不变量 + 总图 + 必要局部架构/流程图 + 每步解释”。html 字段必须是一段可独立嵌入的 HTML 片段，使用内联 <style>、CSS keyframes、SVG、div/span/table，不要使用 JavaScript、script、iframe、外链资源、网络图片或表单。顶层 Mermaid 控制在 5-8 个节点、4-7 条边；局部 Mermaid 控制在 3-6 个节点、2-5 条边；HTML 控制在 4500-30000 字符内。`
  ...
  const example = JSON.stringify({
    ...
    html: '<style>:root{--paper:#fff8ed;--ink:#172033;--blue:#3b82f6;--amber:#f59e0b;--green:#10b981;--coral:#f9735b;--lav:#8b5cf6;--grid:#e8dcc8;--font-ui:"Avenir Next","Helvetica Neue",sans-serif;--font-serif:Georgia,"Times New Roman",serif;--font-mono:SFMono-Regular,"Cascadia Code",monospace}.wb{font-family:var(--font-ui);color:var(--ink);background:linear-gradient(135deg,var(--paper),#eef7ff);padding:32px;border-radius:24px}.hero-svg{width:100%;min-height:430px}.visual-node{fill:#fffdf7;stroke:var(--blue);stroke-width:3;animation:trace .9s both}.path{fill:none;stroke:var(--coral);stroke-width:5;stroke-linecap:round;stroke-dasharray:900;stroke-dashoffset:900;animation:draw 1.4s both forwards}@keyframes trace{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}@keyframes draw{to{stroke-dashoffset:0}}</style><section class="wb" data-html-board="answer-specific"><h2>本题专属 HTML 标题</h2><p class="explain">用文字说明这张图如何读，以及图中每个关键节点代表什么。</p><svg class="hero-svg" viewBox="0 0 1200 560" role="img" aria-label="本题专属大尺寸 SVG 主图"><!-- 必须绘制本轮回答里的具体公式/变量流/流程/架构；所有节点必须显式彩色 fill/stroke，不允许黑色 blob --></svg><section><h3>公式说明</h3><p>解释每个符号、适用条件和边界。</p></section><section><h3>简单示例</h3><p>给一个最小数字或业务例子代入。</p></section><section><h3>动画说明</h3><p>说明动画从哪里开始、依次高亮什么、最后如何读结论。</p></section><table><tr><th>符号/组件/指标</th><th>含义</th><th>在本轮回答中的作用</th></tr></table></section>',
    steps: [
      {
        stage: 'socratic',
        title: '1. Socratic 提问',
...
export function makeWhiteboardFromAnswerPrompt(challenge: Challenge, locale: WhiteboardLocale, answer: string, userMessage = '') {
  ...
  const visualRequirements = zh
    ? 'HTML 视觉要求：默认浅色高质感研究白板，不能是黑色大块动画；禁止默认黑色 SVG fill，禁止浏览器默认字体或 Inter/Roboto/Arial/system-only。主 SVG 必须是大尺寸图，viewBox 至少约 1200x560，宽度 100%，主图高度不少于 420px；图内节点字号 18-28px，公式字号 20-30px。必须定义 :root 色彩 token 和字体 token；至少 8 个具名颜色变量；字体使用 "Avenir Next","Helvetica Neue",sans-serif + Georgia,"Times New Roman",serif + SFMono-Regular,"Cascadia Code",monospace。所有 SVG shape/path/text 必须显式彩色 fill/stroke；路径动画结束后箭头和线条仍完整可见，不能动画完消失。页面必须包含：清晰标题、核心大图、每个节点的文字说明、公式逐项说明、一个简单示例、动画说明、结论/检查清单。动画必须真实存在，用 @keyframes、animation-delay、stroke-dasharray/stroke-dashoffset 或 transform/opacity 让路径/节点/公式按步骤出现；不能只有静态图。文字解释不能少于 6 个说明块，每个关键公式都要解释符号、含义、适用条件和边界，并给一个最小数字或业务例子代入。'
    : 'HTML visual requirements: default to a light premium research-board aesthetic; no black-block animation; no default black SVG fill; no browser-default typography or Inter/Roboto/Arial/system-only. The main SVG must be large, with a viewBox of at least about 1200x560, width 100%, and main diagram height at least 420px; use 18-28px node text and 20-30px formula text. Define :root color and font tokens; include at least 8 named color variables; use "Avenir Next","Helvetica Neue",sans-serif + Georgia,"Times New Roman",serif + SFMono-Regular,"Cascadia Code",monospace. Every SVG shape/path/text must explicitly use colored fill/stroke; animated arrows and lines must remain fully visible after animation, not disappear. The page must include: clear title, large core diagram, text explanation for every key node, formula-by-formula explanation, one simple example, animation explanation, and conclusion/checklist. Animation must be real: use @keyframes, animation-delay, stroke-dasharray/stroke-dashoffset, or transform/opacity so paths/nodes/formulas appear step by step; do not output a static-only diagram. Include at least 6 explanatory text blocks. For every key formula, explain symbols, meaning, applicability, boundary conditions, and include one minimal numeric or business example.'
  const schema = JSON.stringify({
    format: 'steps',
    title: zh ? '基于回答的原理白板' : 'Principle board from answer',
    summary: [zh ? '从学习 Coach 回答抽取核心公式、流程、架构和方法。' : 'Extract formulas, flows, architecture, and methods from the Arena Chat answer.'],
    mermaid: 'flowchart LR\n  Input["answer focus"] --> Mechanism["core mechanism"]\n  Mechanism --> Flow["state / data flow"]\n  Flow --> Check{"boundary / metric"}\n  Check --> Result["conclusion"]',
    html: '<style>:root{--paper:#fff8ed;--ink:#172033;--blue:#3b82f6;--amber:#f59e0b;--green:#10b981;--coral:#f9735b;--lav:#8b5cf6;--grid:#e8dcc8;--font-ui:"Avenir Next","Helvetica Neue",sans-serif;--font-serif:Georgia,"Times New Roman",serif;--font-mono:SFMono-Regular,"Cascadia Code",monospace}.wb{font-family:var(--font-ui);color:var(--ink);background:linear-gradient(135deg,var(--paper),#eef7ff);padding:32px;border-radius:24px}.hero-svg{width:100%;min-height:430px}.node{fill:#fffdf7;stroke:var(--blue);stroke-width:3;animation:pop .7s both}.edge{fill:none;stroke:var(--coral);stroke-width:5;stroke-linecap:round;stroke-dasharray:900;stroke-dashoffset:900;animation:draw 1.3s both forwards}@keyframes pop{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}@keyframes draw{to{stroke-dashoffset:0}}</style><section class="wb" data-html-board="answer-specific"><h2>本题专属 HTML 标题</h2><p class="explain">说明这张动画图如何读。</p><svg class="hero-svg" viewBox="0 0 1200 560" role="img" aria-label="本题专属大尺寸 SVG 主图"></svg><section><h3>公式说明</h3><p>解释符号、含义、适用条件和边界。</p></section><section><h3>简单示例</h3><p>给出最小数字或业务例子代入。</p></section><section><h3>动画说明</h3><p>说明动画依次高亮什么。</p></section><table><tr><th>符号/组件/指标</th><th>含义</th><th>作用</th></tr></table></section>',
    steps: [
