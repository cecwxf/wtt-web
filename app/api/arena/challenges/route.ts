...
function mergeChallenges(primary: Challenge[] | null, fallback: Challenge[]) {
  ...
  for (const challenge of fallback) {
    ...
    // Local AI Kernel / AI Interview seed carries canonical statements and
    // teaching metadata; prefer it over older backend seed rows.
    if ((challenge.category === 'ai-kernel' || challenge.category === 'ai-interview') && existingIndex >= 0) rows[existingIndex] = challenge
